...
class MainController(QMainWindow):
    def __init__(self):
        ...
        self.main_layout.setSpacing(0) # Espacio entre widgets del layout principal
    ...
    def _init_navigation_bar(self):
        ...
        self.nav_bar.setStyleSheet("background-color: #333333; border-top: 1px solid #555555;")
        self.nav_bar_layout = QHBoxLayout(self.nav_bar)
        self.nav_bar_layout.setContentsMargins(10, 5, 10, 5)
        self.nav_bar_layout.setSpacing(20) # Espacio entre los botones de navegación
        ...
        self.nav_bar_layout.addSpacerItem(QSpacerItem(20, 0, QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum))
    ...
    def _init_controllers(self):
        ...
        """
        self.controllers = {} # Diccionario para almacenar los controladores (o placeholders) por nombre
        ...
        self.call_log_view = LlamadasView(self) # Le pasamos 'self' (MainController) como parent
        self.call_log_controller = LlamadasController(self.call_log_model, self.call_log_view) #
        # ^^^^^^^^ CAMBIO AQUÍ ^^^^^^^^
        self.stacked_widget.addWidget(self.call_log_controller.get_view()) # Añade la vista del controlador de llamadas
        self.controllers["llamadas"] = self.call_log_controller

        # 3. Módulo de Ajustes (MVC)
        self.ajustes_model = AjustesModel()
        self.ajustes_view = AjustesView(self) # Le pasamos 'self' (MainController) como parent
        self.ajustes_controller = AjustesController(self.ajustes_model, self.ajustes_view)
        self.stacked_widget.addWidget(self.ajustes_controller.get_view()) # Añade la vista del controlador de ajustes
        ...
        layout_apps.addWidget(QLabel("<h1>Pantalla de Aplicaciones (En construcción)</h1>"))
    ...
    def change_screen(self, screen_name):
        ...
        if screen_name not in self.controllers:
            QMessageBox.critical(self, "Error de Navegación", f"La pantalla '{screen_name}' no está registrada en MainController.")
            return
        ...
        if isinstance(screen_entry, QWidget):
            ...
            if screen_name == "llamadas":
                ...
                target_controller.load_initial_settings() # Esta línea es crucial para cargar los ajustes y datos ADB
    ...
    def go_back(self):
        ...
        if self.view_history:
            ...
            self.nav_back_button.setEnabled(False) # Asegura que el botón esté deshabilitado
